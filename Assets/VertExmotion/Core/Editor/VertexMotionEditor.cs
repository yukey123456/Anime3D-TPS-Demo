...
{ 
    ...
    public class VertExmotionEditor : Editor {
        ...
        public bool m_NeedShaderRefresh = false;
        ...
        public static Shader GetEditorShader()
        {
            ...
            if (shader == null)
                shader = Shader.Find("Hidden/URP/VertExmotion_editor");
            if (shader == null)
    ...
    //			}
            ...
            public void InitializeEditorInstance()
            {
                ...
                if (EditorPrefs.HasKey("VertExmotion_LastMode"))
                {
                    ...
                    m_needRefresh = true;
                    m_NeedShaderRefresh = true;
                }


            m_rgbd = vtm.GetComponent<Rigidbody>();

            if (vtm.m_vertexColors != null)
                vtm.ApplyMotionData();
    ...
    */
    ...
    /*
            ...
            //Clean list if sensors are removed out of the tool
            for (int i = 0; i < vtm.m_VertExmotionSensors.Count; ++i)                
            {
                if (vtm.m_VertExmotionSensors[i] == null)
                    vtm.m_VertExmotionSensors.RemoveAt(i--);
            }
            ...
            */
            ...
            if ( m_mode == eMode.SENSORS && !vtm.m_showEditorPanel)
                UpdateShaderEditor(vtm);
        ...
        */
        ...
        public void DrawUI(VertExmotionBase vtm )
        {
            ...
            m_NeedShaderRefresh = modeChanged;
	    ...
	    void DrawInfo( VertExmotionBase vtm )
	    {
            ...
            GUILayout.Space(5);
            GUILayout.Label("Mode", EditorStyles.centeredGreyMiniLabel);
            GUILayout.BeginHorizontal();

            GUI.color = vtm.m_useVertexBufferMode ? Color.white : Color.grey;
            if (GUILayout.Button("Vertex Buffer", EditorStyles.miniButtonRight))
            {
                vtm.m_useVertexBufferMode = true;
            }

            GUI.color = !vtm.m_useVertexBufferMode ? Color.white : Color.grey;
            if (GUILayout.Button("Shader", EditorStyles.miniButtonLeft))
            {
                vtm.m_useVertexBufferMode = false;
            }

            GUI.color =  Color.white;
            GUILayout.EndHorizontal();

            if (vtm.m_useVertexBufferMode)
            {
                if (!PlayerSettings.gpuSkinning)
                {
                    EditorGUILayout.HelpBox("'GPU Skinning' is disabled in the project settings.", MessageType.Error);
                    if (GUILayout.Button("Enable GPU Skinning"))
                        PlayerSettings.gpuSkinning = true;
                }
                else
                {
                    EditorGUILayout.HelpBox("\nThe 'vertex buffer' mode doesn't require a compatible shader.\n", MessageType.None);
                }
            }
            else
            {
                //EditorGUILayout.HelpBox("\nThe 'Shader' mode apply the modification in the shader.\n", MessageType.None);
            }
#endif
            ...
            */
            ...
            {
                GUILayout.Space(5);
                if (GUILayout.Button("Use Vertex Buffer mode"))
                {
                    vtm.m_useVertexBufferMode = true;
                }
                EditorGUILayout.HelpBox("\nThe 'vertex buffer' mode doesn't require a compatible shader.\n", MessageType.None);
            ...
            if (!warning)
            {
                ...
                GUILayout.Label("   red for motion");
                GUILayout.Label("   blue for static");
                GUILayout.Space(5);
	    ...
	    void DrawPaint( VertExmotionBase vtm )
	    {
            ...
            if (!Application.isPlaying && m_rendererType != eRendererType.SPRITE)
            {
                ...
                vtm.m_params.usePaintDataFromMeshColors = EditorGUILayout.Toggle("Use mesh vertex color", vtm.m_params.usePaintDataFromMeshColors);
               
                //GUILayout.Label("Using the mesh vertex color avoid\nthe mesh duplication in memory,\nwhen there's multiple instances.", EditorStyles.helpBox);
    ...
    //tips
            ...
            if (!Application.isPlaying)
            {
                m_matPropBlk.SetFloat("_SensorId", -1);
                vtm.renderer.SetPropertyBlock(m_matPropBlk);
                if (m_NeedShaderRefresh)
                {
                    vtm.UpdateColorBuffer();
                    m_NeedShaderRefresh = false;
                }
        ...
        void UpdateShaderEditor(VertExmotionBase vtm)
        {
            for (int i = 0; i < vtm.m_VertExmotionSensors.Count; ++i)
            {
        ...
        */
        ...
        public void EnableSceneViewGizmos()
        {
                ...
                {
                    int[] uv = GetUnityVersion();
                    if ((uv[0] >= 2019 && uv[1] >= 3) || uv[0] >= 2020)
                    {
	    ...
	    void DrawBrushMenuContextual( Event e )
	    {
		    ...
		    if( m_enableBrushMenuContextual && m_mode == eMode.PAINT )
		    {
			    Handles.BeginGUI ();
	    ...
	    void DrawCursor( Vector3 mp, Camera svCam )
	    {
            ...
            if (!SceneView.lastActiveSceneView.orthographic)
            {
                ...
                size *= HandleUtility.GetHandleSize(mp) * ((GetUnityVersion()[0] >= 2021 ? 14f : 7f ));
	    ...
	    void OnEnable()
	    {
            ...
            */
			...
			//    DestroyImmediate ( vtm.m_mesh, true );
		    vtm.m_mesh = null;
	    ...
	    void OnDisable()
	    {
           ...
           if (Application.isPlaying)
            ...
            if (vtm.m_MeshReference != null)
                vtm.SetMesh(vtm.m_MeshReference);
