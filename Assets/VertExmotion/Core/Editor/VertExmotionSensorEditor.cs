...
//			m_fadePanel [(int)mode - 1] = Mathf.Clamp (m_fadePanel [(int)mode - 1], 0.001f, 1f);
		...
		public static bool DrawSensorHandle( VertExmotionSensorBase sensor, bool AllInfo )
		{
            ...
            if (m_settingMode == eSettingsMode.SENSORS || m_settingMode == eSettingsMode.FX )
            {
                ...
                sensor.transform.position = Handles.FreeMoveHandle(sensor.transform.position, (constUnit * .02f), Vector3.zero, CircleCap);
#else
                sensor.transform.position = Handles.FreeMoveHandle(sensor.transform.position, Quaternion.identity, (constUnit * .02f), Vector3.zero, CircleCap);
#endif
...
#if UNITY_2022_1_OR_NEWER                
                lastPos = Handles.FreeMoveHandle(lastPos, (constUnit * .01f), Vector3.zero, CircleCap);
#else
...
#if UNITY_2022_1_OR_NEWER
						vtmCol.m_collisionZones[i].positionOffset = vtmCol.transform.InverseTransformPoint( Handles.FreeMoveHandle( worldColZonePos, ( constUnit*.02f ), Vector3.zero, CircleCap ) );
#else
