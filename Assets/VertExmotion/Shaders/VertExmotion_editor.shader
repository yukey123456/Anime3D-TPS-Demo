...
Shader "Hidden/VertExmotion_editor" {
	...
	CGPROGRAM
		...
		void vert (appdata_full v, uint vertexID : SV_VertexID, out vertexToFragment o) {

			
			float4 vColor = _colorBuf[vertexID];
...
//#endif
			...
			for (int i = 0; i < NB_SENSORS; ++i)			
			{
				if (i < KVM_NbSensors)
				{
					...
					if (_LayerId == 0)
						layerColor = float3(1, 1, 1) * max(max(vColor.r, vColor.g), vColor.b);
					if (_LayerId == 1)
						layerColor = vColor.rrr;
					if (_LayerId == 2)
						layerColor = vColor.ggg;
					if (_LayerId == 3)
						layerColor = vColor.bbb;
...
/*
	...
	*/					
		...
		else
		{
			//paint info
			if (_LayerId == 0)
				o.color.rgb = float3(1, 1, 1) * max(max(vColor.r, vColor.g), vColor.b);
			if (_LayerId == 1)
				o.color.rgb = vColor.rrr;
			if (_LayerId == 2)
				o.color.rgb = vColor.ggg;
			if (_LayerId == 3)
				o.color.rgb = vColor.bbb;
...
//#ifdef VERTEXMOTION_GRADIENT_ON
		...
		c1 = clamp(c1,float4(0, 0, 0, 0), float4(1, 1, 1, 1));
